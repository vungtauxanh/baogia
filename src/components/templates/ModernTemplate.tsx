  ...
  const renderComponent = (componentId: string, rowId: string, colIdx: number) => {
    ...
    switch (componentId) {
            ...
            <div className="flex-1">
              {businessInfo.logoPosition === 'inline-left' ? (
                <div className={`flex items-center gap-4 ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'right' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div>
                    <h1 className="font-bold text-3xl text-blue-700 uppercase mb-2">{businessInfo.name}</h1>
      ...
      case 'title':
        return componentWrapper(
          <div key="title">
            <h2 className="font-black text-4xl text-gray-200 uppercase tracking-wider">{isHandoverMode ? "BIÊN BẢN BÀN GIAO" : "BÁO GIÁ"}</h2>
          </div>,
          rowId, colIdx
        );
      case 'date':
      ...
      case 'customer':
        return componentWrapper(
          <div key="customer" className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            <p className="text-sm text-blue-600 font-semibold mb-1">KÍNH GỬI:</p>
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-right font-semibold text-blue-700">{formatCurrency(amount)}</td>
                        </>
                      )}
      ...
      case 'totals':
        ...
        return componentWrapper(
          <div key="totals" className={`flex ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'left' ? 'justify-start' : 'justify-end'}`}>
            <div className="w-full max-w-md bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-2">
              ...
              <div className="flex justify-between items-center font-bold text-xl text-gray-800 pt-2">
                <span>TỔNG CỘNG:</span>
                <span className="text-blue-700">{formatCurrency(data.total)} VNĐ</span>
              </div>
            </div>
          </div>,
          rowId, colIdx
