...
export const CorporateTemplate: React.FC<TemplateProps> = ({ 
    ...
    switch (componentId) {
      case 'business':
        ...
        return componentWrapper(
          <div key="business" className={`flex justify-between items-center mb-0 border-b-4 border-slate-800 pb-6 ${businessHeaderAlignClass}`}>
            <div className="flex-1">
              {businessInfo.logoPosition === 'inline-left' ? (
                <div className={`flex items-center gap-4 ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'right' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div>
                    <h1 className="font-bold text-2xl text-slate-900 uppercase tracking-wide">{businessInfo.name}</h1>
                    <p className="text-slate-500 text-sm">{businessInfo.address}</p>
                  </div>
                </div>
              ) : (
                <>
                  {businessInfo.logo ? (
                    ...
                    <div className={`w-16 h-16 bg-slate-800 text-white flex items-center justify-center font-bold text-2xl rounded-sm mb-2 ${businessLogoAlignClass}`}>
      ...
      case 'title':
        return componentWrapper(
          <div key="title">
            <h2 className="font-bold text-3xl text-slate-800 uppercase tracking-widest">{isHandoverMode ? "BIÊN BẢN BÀN GIAO" : "BÁO GIÁ"}</h2>
          </div>,
          rowId, colIdx
        );
      case 'date':
      ...
      case 'customer':
        return componentWrapper(
          <div key="customer" className="mb-8">
            <div className={`w-full ${settings.alignment === 'center' ? 'text-center' : settings.alignment === 'right' ? 'text-right' : 'text-left'}`}>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Thông tin khách hàng</h3>
              <p className="font-bold text-lg text-slate-800 uppercase">{customerName}</p>
              {data.customerAddress && <p className="text-slate-600 mt-1">{data.customerAddress}</p>}
            </div>
          </div>,
          rowId, colIdx
        );
            ...
            <table className="w-full border-collapse table-fixed">
              <thead>
                <tr className="bg-slate-100 border-y-2 border-slate-300">
                  <th className="py-3 px-2 text-center font-semibold text-slate-700" style={{ width: `${columnWidths?.stt || 5}%` }}>STT</th>
                  {businessInfo.showItemImages && <th className="py-3 px-2 text-center w-20 font-semibold text-slate-700">Hình</th>}
                  <th className="py-3 px-2 text-left font-semibold text-slate-700" style={{ width: `${columnWidths?.name || 40}%` }}>Mô tả hàng hóa / Dịch vụ</th>
                  <th className="py-3 px-2 text-center font-semibold text-slate-700" style={{ width: `${columnWidths?.unit || 10}%` }}>ĐVT</th>
                  <th className="py-3 px-2 text-center font-semibold text-slate-700" style={{ width: `${columnWidths?.quantity || 10}%` }}>SL</th>
                  {!isHandoverMode && (
                    <>
                      <th className="py-3 px-2 text-right font-semibold text-slate-700" style={{ width: `${columnWidths?.price || 15}%` }}>Đơn giá</th>
                  <th className="py-3 px-2 text-right font-semibold text-slate-700" style={{ width: `${columnWidths?.amount || 20}%` }}>Thành tiền</th>
                    </>
                  )}
                </tr>
              </thead>
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    <Reorder.Item 
                      ...
                      className="border-b border-slate-200 group/row cursor-default bg-white"
                    >
                      ...
                      {businessInfo.showItemImages && (
                        <td className="py-3 px-2 text-center">
                          {item.image && <img src={item.image} alt={item.name} className="w-10 h-10 object-contain mx-auto" />}
                        </td>
                      )}
                      <td className="py-3 px-2">
                        <p className="font-medium text-slate-800 whitespace-pre-wrap">{item.name}</p>
                      ...
                      <td className="py-3 px-2 text-right font-medium text-slate-800">{formatCurrency(amount)}</td>
                        </>
                      )}
                    </Reorder.Item>
                  );
        ...
        return componentWrapper(
          <div key="totals" className={`flex ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'left' ? 'justify-start' : 'justify-end'}`}>
            <div className="w-full max-w-md">
              ...
              <div className="flex justify-between py-2 border-b border-slate-200">
                <span className="text-slate-600">Thuế GTGT {data.isVatIncluded ? '' : `(${data.vatRate}%):`}</span>
                <span className="font-medium">{data.isVatIncluded ? 'Đã bao gồm' : formatCurrency(data.subtotal * (data.vatRate / 100))}</span>
              </div>
              <div className="flex justify-between py-3 bg-slate-800 text-white px-4 mt-2 rounded-sm">
                <span className="font-bold uppercase">Tổng cộng:</span>
                <span className="font-bold">{formatCurrency(data.total)} VNĐ</span>
              </div>
            </div>
          </div>,
          rowId, colIdx
      ...
      case 'payment':
        return componentWrapper(
          <div key="payment" className={`text-sm text-slate-600 ${settings.alignment === 'center' ? 'text-center' : settings.alignment === 'right' ? 'text-right' : 'text-left'}`}>
            <p className="font-bold text-slate-800 mb-1">Thông tin thanh toán:</p>
      ...
      case 'notes':
        return componentWrapper(
          <div key="notes" className={`text-sm text-slate-600 ${settings.alignment === 'center' ? 'text-center' : settings.alignment === 'right' ? 'text-right' : 'text-left'}`}>
            <p className="font-bold text-slate-800 mb-1">Ghi chú:</p>
            {businessInfo.showNotes && (
              <div>
                {businessInfo.notes ? (
                  <div className="whitespace-pre-wrap">{businessInfo.notes}</div>
                ) : (
                  <p>- Báo giá có giá trị trong 30 ngày.</p>
                )}
              </div>
            )}
          </div>,
  ...
  return (
    <div 
      className={`bg-white shadow-2xl ${containerStyle} font-sans text-[14px] leading-relaxed print:shadow-none print:w-full print:min-h-0 print:p-0 text-slate-800 relative group/template`}
      style={marginStyle}
    >
