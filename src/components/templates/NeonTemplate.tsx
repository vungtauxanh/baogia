...
export const NeonTemplate: React.FC<TemplateProps> = ({ 
  ...
  const renderComponent = (componentId: string, rowId: string, colIdx: number) => {
    ...
    const componentWrapper = (content: React.ReactNode, rowId: string, colIdx: number) => (
      <div className={`relative group/comp w-full ${alignmentClass} focus:outline-none`} tabIndex={0}>
        <div className="absolute -top-6 left-0 flex items-center gap-1 opacity-0 group-hover/comp:opacity-100 group-focus-within/comp:opacity-100 group-focus/comp:opacity-100 transition-opacity print:hidden z-30 bg-gray-800 border rounded shadow-sm p-1">
          ...
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'left' ? 'bg-blue-100 text-pink-500' : 'hover:bg-gray-100'}`}
          >
            <AlignLeft size={12} />
          </button>
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'center' ? 'bg-blue-100 text-pink-500' : 'hover:bg-gray-100'}`}
          >
            <AlignCenter size={12} />
          </button>
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'right' ? 'bg-blue-100 text-pink-500' : 'hover:bg-gray-100'}`}
          >
            <AlignRight size={12} />
          </button>
          <div className="w-px h-3 bg-gray-200 mx-1"></div>
          <select 
            ...
            className="text-[10px] border-none bg-transparent focus:ring-0 p-0 cursor-pointer font-bold text-pink-500"
          >
    ...
    switch (componentId) {
            ...
            <div className="flex-1">
              {businessInfo.logoPosition === 'inline-left' ? (
                <div className={`flex items-center gap-4 ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'right' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div>
                    <h1 className="font-bold text-3xl text-pink-500 uppercase mb-2">{businessInfo.name}</h1>
                    <p className="text-gray-300">{businessInfo.address}</p>
                    <p className="text-gray-300">MST: {businessInfo.taxCode} | ĐT: {businessInfo.phone}</p>
                  </div>
                </div>
              ) : (
                <>
      ...
      case 'title':
        return componentWrapper(
          <div key="title">
            <h2 className="font-black text-5xl text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500 drop-shadow-[0_0_10px_rgba(236,72,153,0.8)] text-gray-200 uppercase tracking-wider">{isHandoverMode ? "BIÊN BẢN BÀN GIAO" : "BÁO GIÁ"}</h2>
          </div>,
          rowId, colIdx
        );
      case 'date':
      ...
      case 'customer':
        return componentWrapper(
          <div key="customer" className="bg-pink-50 p-4 rounded-lg border border-blue-100">
            <p className="text-sm text-pink-500 font-semibold mb-1">KÍNH GỬI:</p>
            <p className="font-bold text-lg text-gray-100 uppercase">{customerName}</p>
            ...
            <table className="w-full table-fixed">
              <thead>
                <tr className="bg-pink-500 text-white">
                  <th className="p-3 text-center rounded-tl-lg" style={{ width: `${columnWidths?.stt || 5}%` }}>STT</th>
                  ...
                  <th className="p-3 text-left w-24 rounded-tr-lg">Ghi chú</th>
                </tr>
              </thead>
                ...
                {items.map((item, index) => {
                  ...
                  if (item.isTextOnly) {
                    return (
                      <Reorder.Item 
                        ...
                        value={item} 
                        as="tr" 
                        className="group/row cursor-default bg-gray-800 border-b border-gray-700"
                      >
                        <td colSpan={isHandoverMode ? (businessInfo.showItemImages ? 5 : 4) : (businessInfo.showItemImages ? 7 : 6)} className="p-3 relative">
                          <div className="absolute left-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 transition-opacity print:hidden cursor-grab active:cursor-grabbing p-1 bg-pink-50 rounded border border-pink-200 -left-6">
                  ...
                  return (
                    <Reorder.Item 
                      ...
                      className="border-b border-gray-700 hover:bg-gray-900 text-white group/row cursor-default bg-gray-800"
                    >
                      <td className="p-3 text-center text-gray-400 relative">
                        {index + 1}
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 transition-opacity print:hidden cursor-grab active:cursor-grabbing p-1 bg-pink-50 rounded border border-pink-200 -left-4">
                          <GripVertical size={14} className="text-pink-500" />
                        </div>
                      </td>
                      ...
                      <td className="p-3 font-medium text-gray-100 whitespace-pre-wrap">{item.name}</td>
                      <td className="p-3 text-center text-gray-300">{item.unit}</td>
                      ...
                      <td className="p-3 text-right font-semibold text-pink-500">{formatCurrency(amount)}</td>
                        </>
                      )}
                      <td className="p-3 text-sm text-gray-400">{item.note}</td>
                    </Reorder.Item>
                  );
                })}
              </Reorder.Group>
      ...
      case 'totals':
        ...
        return componentWrapper(
          <div key="totals" className={`flex ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'left' ? 'justify-start' : 'justify-end'}`}>
            <div className="w-full max-w-md bg-gray-900 text-white p-4 rounded-lg border border-gray-700 space-y-2">
              <div className="flex justify-between items-center text-gray-300">
                <span>Cộng tiền hàng:</span>
                <span className="font-medium">{formatCurrency(data.subtotal)} VNĐ</span>
              </div>
              <div className="flex justify-between items-center text-gray-300 border-b border-gray-700 pb-2">
              ...
              <div className="flex justify-between items-center font-bold text-xl text-gray-100 pt-2">
                <span>TỔNG CỘNG:</span>
                <span className="text-pink-500">{formatCurrency(data.total)} VNĐ</span>
              </div>
            </div>
          </div>,
          rowId, colIdx
      ...
      case 'payment':
        return componentWrapper(
          <div key="payment" className={`text-sm text-gray-300 ${settings.alignment === 'center' ? 'text-center' : settings.alignment === 'right' ? 'text-right' : 'text-left'}`}>
            <h3 className="font-bold text-gray-100 mb-2">Thông tin thanh toán</h3>
      ...
      case 'notes':
        return componentWrapper(
          <div key="notes" className={`text-sm text-gray-300 ${settings.alignment === 'center' ? 'text-center' : settings.alignment === 'right' ? 'text-right' : 'text-left'}`}>
            <h3 className="font-bold text-gray-100 mb-2">Ghi chú</h3>
    ...
    return (
      <div className="relative group/row-layout focus:outline-none" tabIndex={0}>
        <div className="absolute -left-32 top-0 flex flex-col gap-1 opacity-0 group-hover/row-layout:opacity-100 group-focus-within/row-layout:opacity-100 group-focus/row-layout:opacity-100 transition-opacity print:hidden z-20">
          ...
          <div className="flex bg-gray-800 border rounded shadow-sm p-1 gap-1 items-center relative">
            <div className="flex flex-col gap-1">
              <button 
                ...
                className="p-1 hover:bg-gray-100 rounded text-pink-500"
                title="Thêm cột"
              >
                <Plus size={14} />
              </button>
            ...
            <div className="flex flex-col items-center gap-1 group/spacing focus:outline-none relative" tabIndex={0}>
              <Settings size={18} className="text-gray-400 pointer-events-none" />
              <div 
                className="absolute right-full mr-2 top-0 bg-gray-800 border rounded shadow-lg p-3 hidden group-hover/spacing:block group-focus-within/spacing:block group-focus/spacing:block z-50 w-48 cursor-default" 
  ...
  return (
    <div 
      className={`bg-gray-800 shadow-2xl ${containerStyle} font-sans text-[14px] leading-relaxed print:shadow-none print:w-full print:min-h-0 print:p-0 relative group/template`}
      style={marginStyle}
    >
      ...
      <div className="mt-8 pt-4 border-t border-dashed border-gray-700 print:hidden">
        <button 
          onClick={() => {
          ...
          className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-400 hover:text-pink-500 hover:border-blue-300 transition-colors flex items-center justify-center gap-2"
        >
