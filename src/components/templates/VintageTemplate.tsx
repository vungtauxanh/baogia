...
export const VintageTemplate: React.FC<TemplateProps> = ({ 
  data, 
  onReorder, 
  onRowSpacingChange, 
  ...
  const renderComponent = (componentId: string, rowId: string, colIdx: number) => {
      ...
      <div className={`relative group/comp w-full ${alignmentClass} focus:outline-none`} tabIndex={0}>
        <div className="absolute -top-6 left-0 flex items-center gap-1 opacity-0 group-hover/comp:opacity-100 group-focus-within/comp:opacity-100 group-focus/comp:opacity-100 transition-opacity print:hidden z-30 bg-white border rounded shadow-sm p-1">
          ...
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'left' ? 'bg-blue-100 text-amber-800' : 'hover:bg-gray-100'}`}
          >
            <AlignLeft size={12} />
          </button>
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'center' ? 'bg-blue-100 text-amber-800' : 'hover:bg-gray-100'}`}
          >
            <AlignCenter size={12} />
          </button>
          <button 
            ...
            className={`p-1 rounded ${settings.alignment === 'right' ? 'bg-blue-100 text-amber-800' : 'hover:bg-gray-100'}`}
          >
            <AlignRight size={12} />
          </button>
          <div className="w-px h-3 bg-gray-200 mx-1"></div>
          <select 
            ...
            className="text-[10px] border-none bg-transparent focus:ring-0 p-0 cursor-pointer font-bold text-amber-800"
          >
    ...
    switch (componentId) {
            ...
            <div className="flex-1">
              {businessInfo.logoPosition === 'inline-left' ? (
                <div className={`flex items-center gap-4 ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'right' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div>
                    <h1 className="font-bold text-3xl text-amber-800 uppercase mb-2">{businessInfo.name}</h1>
      ...
      case 'title':
        return componentWrapper(
          <div key="title">
            <h2 className="font-serif text-4xl text-amber-900 tracking-widest border-b-2 border-amber-900 pb-2 inline-block text-gray-200 uppercase tracking-wider">{isHandoverMode ? "BIÊN BẢN BÀN GIAO" : "BÁO GIÁ"}</h2>
          </div>,
          rowId, colIdx
        );
      case 'date':
      ...
      case 'customer':
        return componentWrapper(
          <div key="customer" className="bg-amber-50 p-4 rounded-none border border-blue-100">
            <p className="text-sm text-amber-800 font-semibold mb-1">KÍNH GỬI:</p>
                ...
                {items.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        <td colSpan={isHandoverMode ? (businessInfo.showItemImages ? 5 : 4) : (businessInfo.showItemImages ? 7 : 6)} className="p-3 relative">
                          <div className="absolute left-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 transition-opacity print:hidden cursor-grab active:cursor-grabbing p-1 bg-amber-50 rounded border border-amber-200 -left-6">
                  ...
                  return (
                    <Reorder.Item 
                      ...
                      className="border-b border-gray-200 hover:bg-[#fdfbf7] group/row cursor-default bg-white"
                    >
                      <td className="p-3 text-center text-gray-500 relative">
                        {index + 1}
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 transition-opacity print:hidden cursor-grab active:cursor-grabbing p-1 bg-amber-50 rounded border border-amber-200 -left-4">
                          <GripVertical size={14} className="text-amber-500" />
                        </div>
                      </td>
                      ...
                      <td className="p-3 text-right font-semibold text-amber-800">{formatCurrency(amount)}</td>
                        </>
                      )}
      ...
      case 'totals':
        ...
        return componentWrapper(
          <div key="totals" className={`flex ${settings.alignment === 'center' ? 'justify-center' : settings.alignment === 'left' ? 'justify-start' : 'justify-end'}`}>
            <div className="w-full max-w-md bg-[#fdfbf7] p-4 rounded-none border border-gray-200 space-y-2">
              ...
              <div className="flex justify-between items-center font-bold text-xl text-gray-800 pt-2">
                <span>TỔNG CỘNG:</span>
                <span className="text-amber-800">{formatCurrency(data.total)} VNĐ</span>
              </div>
            </div>
          </div>,
          rowId, colIdx
          ...
          <div className="flex bg-white border rounded shadow-sm p-1 gap-1 items-center relative">
            <div className="flex flex-col gap-1">
              <button 
                ...
                className="p-1 hover:bg-gray-100 rounded text-amber-800"
                title="Thêm cột"
              >
                <Plus size={14} />
              </button>
    ...
    >
      ...
      <div className="mt-8 pt-4 border-t border-dashed border-gray-200 print:hidden">
        <button 
          ...
          className="w-full py-2 border-2 border-dashed border-gray-300 rounded-none text-gray-400 hover:text-amber-500 hover:border-blue-300 transition-colors flex items-center justify-center gap-2"
        >
